...
function moveBall() {
    ...
    // Paddle collision with BOTTOM PADLE
    if (ball.x - ball.size > paddle.x && ball.x + ball.size < paddle.x + paddle.w && ball.y + ball.size > paddle.y) {
        ball.dy = -ball.speed;
    }
    // Paddle collision with UPPER PADDLE
    if (ball.x - ball.size <= newPaddle.x + newPaddle.w && ball.x + ball.size >= newPaddle.x - newPaddle.w && ball.y + ball.size === newPaddle.y + 2 * newPaddle.h) {
        ball.dy = ball.speed;
    }

    // Score increise, bottom wall
    if (ball.y + ball.size > canvas.height) {
        // Here you changed code !!!
...
function keyUp(e) {
    ...
    if (e.key === 'q' || e.key === 'a' || e.key === 'e' || e.key === 'd') {
        newPaddle.dx = 0;
    }
}
